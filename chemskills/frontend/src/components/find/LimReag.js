...
function limReag() {
    let elm1 = metal();
    let elm2 = nonmetal();
    let chg1 = charge(elm1);
    let chg2 = charge(elm2);
    let m1 = (Math.floor(Math.random() * 5) + 1) * Molar_mass[Elements[elm1]] + Math.floor(Math.random() * 50) + 1;
    let m2 = (Math.floor(Math.random() * 5) + 1) * Molar_mass[Elements[elm2]] + Math.floor(Math.random() * 50) + 1;
    let count1 = 1;
    let count2 = 1;

    while (count1 * chg1 + count2 * chg2 !== 0) {
        if (Math.abs(count1 * chg1) < Math.abs(count2 * chg2)) {
            count1 += 1;
        } else {
            count2 += 1;
        }
    ...
    let user_input = prompt(`If ${m1} grams of ${Elements[elm1]} reacted with ${m2} grams of ${Elements[elm2]}, creating ${Elements[elm1]}(${count1})${Elements[elm2]}(${count2}), which element is the limiting reagent?`);


    let ans;
    if (m1 / Molar_mass[Elements[elm1]] > m2 / Molar_mass[Elements[elm2]]) {
        ans = Elements[elm1];
    } else if (m1 / Molar_mass[Elements[elm1]] === m2 / Molar_mass[Elements[elm2]]) {
        ans = "N/A";
    } else {
        ans = Elements[elm2];
    }
    ...
    if (user_input === ans) {
        ...
        alert("incorrect, the answer is: " + ans);
    }
}
