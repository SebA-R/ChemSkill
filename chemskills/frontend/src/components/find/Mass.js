...
function find_mass() {
    ...
    let mol = Math.floor(Math.random() * 5) + 1;

    let ans = mol * Molar_mass[Elements[elm]];
    ans = Math.round(ans * 10) / 10; // Rounding to the tenth decimal place

    let user_input = prompt("What is the mass of " + mol + Elements[elm] + "? (round to the tenth decimal place)");

    if (user_input === ans) {
        alert("Correct");
    } else {
        alert("Incorrect, the answer is: " + ans + "g");
    }
}
